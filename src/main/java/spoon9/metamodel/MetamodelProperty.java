...
public class MetamodelProperty {
	...
	MMMethod addMethod(CtMethod<?> method, boolean createIfNotExist) {
		...
		if (createIfNotExist) {
			...
			Metamodel.getOrCreate(methodsByKind, mmMethod.getKind(), () -> new ArrayList<>()).add(mmMethod);
	...
	void addSuperField(MetamodelProperty superMMField) {
		if (Metamodel.addUniqueObject(superProperties, superMMField)) {
	...
	public MetamodelProperty getSuperProperty() {
		...
		superProperties.forEach(superField -> {
			Metamodel.addUniqueObject(potentialRootSuperFields, superField.getSuperProperty());
		});
		int idx = 0;
