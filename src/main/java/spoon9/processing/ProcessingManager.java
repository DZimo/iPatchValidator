 ...
 * {@link Factory} with a set of processors. The program model has
 * been previously built using a {@link SpoonModelBuilder} - see
 * {@link SpoonModelBuilder#build()}. To use, add processors to
 * the manager, and then call the {@code process} method. Also, the method
 * {@link Processor#processingDone()} is up called.
 *
 * @see Environment#getManager()
 */
