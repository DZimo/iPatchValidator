...
public class CtForImpl extends CtLoopImpl implements CtFor {
	...
	public <T extends CtFor> T addForInit(CtStatement statement) {
		...
		if (forInit == CtElementImpl.<CtStatement>emptyList()) {
			forInit = new ArrayList<>(ModelElementContainerDefaultCapacities.FOR_INIT_STATEMENTS_CONTAINER_DEFAULT_CAPACITY);
		}
		statement.setParent(this);
		getFactory().getEnvironment().getModelChangeListener().onListAdd(this, CtRole.FOR_INIT, this.forInit, statement);
	...
	public boolean removeForInit(CtStatement statement) {
		...
		getFactory().getEnvironment().getModelChangeListener().onListDelete(this, CtRole.FOR_INIT, forInit, forInit.indexOf(statement), statement);
	...
	public <T extends CtFor> T addForUpdate(CtStatement statement) {
		...
		if (forUpdate == CtElementImpl.<CtStatement>emptyList()) {
			forUpdate = new ArrayList<>(ModelElementContainerDefaultCapacities.FOR_UPDATE_STATEMENTS_CONTAINER_DEFAULT_CAPACITY);
		}
		statement.setParent(this);
		getFactory().getEnvironment().getModelChangeListener().onListAdd(this, CtRole.FOR_UPDATE, this.forUpdate, statement);
	...
	public boolean removeForUpdate(CtStatement statement) {
		...
		getFactory().getEnvironment().getModelChangeListener().onListDelete(this, CtRole.FOR_UPDATE, forUpdate, forUpdate.indexOf(statement), statement);
