...
public class CtAssignmentImpl<T, A extends T> extends CtStatementImpl implements CtAssignment<T, A> {
	...
	public <C extends CtAssignment<T, A>> C setAssigned(CtExpression<T> assigned) {
		...
		getFactory().getEnvironment().getModelChangeListener().onObjectUpdate(this, CtRole.ASSIGNED, assigned, this.assigned);
	...
	public <C extends CtRHSReceiver<A>> C setAssignment(CtExpression<A> assignment) {
		...
		getFactory().getEnvironment().getModelChangeListener().onObjectUpdate(this, CtRole.ASSIGNMENT, assignment, this.assignment);
	...
	public <C extends CtExpression<T>> C addTypeCast(CtTypeReference<?> type) {
		...
		type.setParent(this);
		getFactory().getEnvironment().getModelChangeListener().onListAdd(this, CtRole.CAST, typeCasts, type);
		typeCasts.add(type);
		return (C) this;
	}
