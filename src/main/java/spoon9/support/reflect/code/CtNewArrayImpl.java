...
public class CtNewArrayImpl<T> extends CtExpressionImpl<T> implements CtNewArray<T> {
	...
	public <C extends CtNewArray<T>> C addDimensionExpression(CtExpression<Integer> dimension) {
		...
		if (dimensionExpressions == CtElementImpl.<CtExpression<Integer>>emptyList()) {
			dimensionExpressions = new ArrayList<>(ModelElementContainerDefaultCapacities.NEW_ARRAY_DEFAULT_EXPRESSIONS_CONTAINER_DEFAULT_CAPACITY);
		}
		dimension.setParent(this);
		getFactory().getEnvironment().getModelChangeListener().onListAdd(this, CtRole.DIMENSION, this.dimensionExpressions, dimension);
	...
	public boolean removeDimensionExpression(CtExpression<Integer> dimension) {
		...
		getFactory().getEnvironment().getModelChangeListener().onListDelete(this, CtRole.DIMENSION, dimensionExpressions, dimensionExpressions.indexOf(dimension), dimension);
	...
	public <C extends CtNewArray<T>> C addElement(CtExpression<?> expression) {
		...
		getFactory().getEnvironment().getModelChangeListener().onListAdd(this, CtRole.EXPRESSION, this.expressions, expression);
	...
	public boolean removeElement(CtExpression<?> expression) {
		...
		getFactory().getEnvironment().getModelChangeListener().onListDelete(this, CtRole.EXPRESSION, expressions, expressions.indexOf(expression), expression);
