...
import spoon9.support.util.internal.MapUtils;
...
public class CtTypeReferenceImpl<T> extends CtReferenceImpl implements CtTypeReference<T> {
	...
	public <C extends CtActualTypeContainer> C setActualTypeArguments(List<? extends CtTypeReference<?>> actualTypeArguments) {
		...
		if (this.actualTypeArguments == CtElementImpl.<CtTypeReference<?>>emptyList()) {
			this.actualTypeArguments = new ArrayList<>(ModelElementContainerDefaultCapacities.TYPE_TYPE_PARAMETERS_CONTAINER_DEFAULT_CAPACITY);
		}
	...
	public <C extends CtTypeReference<T>> C setDeclaringType(CtTypeReference<?> declaringType) {
		...
		getFactory().getEnvironment().getModelChangeListener().onObjectUpdate(this, CtRole.DECLARING_TYPE, declaringType, this.declaringType);
	...
	public <C extends CtTypeReference<T>> C setPackage(CtPackageReference pack) {
		...
		getFactory().getEnvironment().getModelChangeListener().onObjectUpdate(this, CtRole.PACKAGE_REF, pack, this.pack);
		this.pack = pack;
		return (C) this;
	}
	...
	public <C extends CtActualTypeContainer> C addActualTypeArgument(CtTypeReference<?> actualTypeArgument) {
		...
		if (actualTypeArguments == CtElementImpl.<CtTypeReference<?>>emptyList()) {
			actualTypeArguments = new ArrayList<>(ModelElementContainerDefaultCapacities.TYPE_TYPE_PARAMETERS_CONTAINER_DEFAULT_CAPACITY);
		}
		actualTypeArgument.setParent(this);
		getFactory().getEnvironment().getModelChangeListener().onListAdd(this, CtRole.TYPE_ARGUMENT, this.actualTypeArguments, actualTypeArgument);
	...
	public boolean removeActualTypeArgument(CtTypeReference<?> actualTypeArgument) {
		...
		getFactory().getEnvironment().getModelChangeListener().onListDelete(this, CtRole.TYPE_ARGUMENT, actualTypeArguments, actualTypeArguments.indexOf(actualTypeArgument), actualTypeArgument);
