...
import spoon9.support.util.QualifiedNameBasedSortedSet;
...
public abstract class CtExecutableImpl<R> extends CtNamedElementImpl implements CtExecutable<R> {
	...
	public <T extends CtExecutable<R>> T setParameters(List<CtParameter<?>> parameters) {
		...
		if (this.parameters == CtElementImpl.<CtParameter<?>>emptyList()) {
			this.parameters = new ArrayList<>(ModelElementContainerDefaultCapacities.PARAMETERS_CONTAINER_DEFAULT_CAPACITY);
		}
	...
	public <T extends CtExecutable<R>> T addParameterAt(int position, CtParameter<?> parameter) {
		...
		if (parameters == CtElementImpl.<CtParameter<?>>emptyList()) {
			parameters = new ArrayList<>(ModelElementContainerDefaultCapacities.PARAMETERS_CONTAINER_DEFAULT_CAPACITY);
		}
		parameter.setParent(this);
		getFactory().getEnvironment().getModelChangeListener().onListAdd(this, CtRole.PARAMETER, this.parameters, parameter);
	...
	public boolean removeParameter(CtParameter<?> parameter) {
		...
		getFactory().getEnvironment().getModelChangeListener().onListDelete(this, CtRole.PARAMETER, parameters, parameters.indexOf(parameter), parameter);
	...
	public <T extends CtExecutable<R>> T setThrownTypes(Set<CtTypeReference<? extends Throwable>> thrownTypes) {
		...
		getFactory().getEnvironment().getModelChangeListener().onSetDeleteAll(this, CtRole.THROWN, this.thrownTypes, new HashSet<Object>(this.thrownTypes));
	...
	public <T extends CtExecutable<R>> T addThrownType(CtTypeReference<? extends Throwable> throwType) {
		...
		getFactory().getEnvironment().getModelChangeListener().onSetAdd(this, CtRole.THROWN, this.thrownTypes, throwType);
	...
	public boolean removeThrownType(CtTypeReference<? extends Throwable> throwType) {
		...
		getFactory().getEnvironment().getModelChangeListener().onSetDelete(this, CtRole.THROWN, thrownTypes, throwType);
