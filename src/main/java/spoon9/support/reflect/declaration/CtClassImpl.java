...
import spoon9.support.compiler.jdt.JDTBasedSpoonCompiler;
import spoon9.support.reflect.code.CtStatementImpl;
import spoon9.support.reflect.eval.VisitorPartialEvaluator;
...
public class CtClassImpl<T> extends CtTypeImpl<T> implements CtClass<T> {
	...
	public <C extends CtClass<T>> C addAnonymousExecutable(CtAnonymousExecutable e) {
		...
		e.setParent(this);
		getFactory().getEnvironment().getModelChangeListener().onListAdd(this, CtRole.ANNONYMOUS_EXECUTABLE, typeMembers, e);
		return addTypeMember(e);
	}

	@Override
	public boolean removeAnonymousExecutable(CtAnonymousExecutable e) {
		getFactory().getEnvironment().getModelChangeListener().onListDelete(this, CtRole.ANNONYMOUS_EXECUTABLE, typeMembers, typeMembers.indexOf(e), e);
	...
	public <C extends CtClass<T>> C setAnonymousExecutables(List<CtAnonymousExecutable> anonymousExecutables) {
		getFactory().getEnvironment().getModelChangeListener().onListDelete(this, CtRole.ANNONYMOUS_EXECUTABLE, typeMembers, new ArrayList<>(getAnonymousExecutables()));
	...
	public <C extends CtClass<T>> C setConstructors(Set<CtConstructor<T>> constructors) {
		...
		getFactory().getEnvironment().getModelChangeListener().onListDelete(this, CtRole.CONSTRUCTOR, typeMembers, oldConstructor);
	...
	public <C extends CtClass<T>> C addConstructor(CtConstructor<T> constructor) {
		getFactory().getEnvironment().getModelChangeListener().onListAdd(this, CtRole.CONSTRUCTOR, typeMembers, constructor);
	...
	public <C extends CtType<T>> C setSuperclass(CtTypeReference<?> superClass) {
		...
		getFactory().getEnvironment().getModelChangeListener().onObjectUpdate(this, CtRole.SUPER_TYPE, superClass, this.superClass);
