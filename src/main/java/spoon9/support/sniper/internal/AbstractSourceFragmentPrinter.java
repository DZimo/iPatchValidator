...
abstract class AbstractSourceFragmentPrinter implements SourceFragmentPrinter {
	...
	protected void printOriginSpacesUntilFragmentIndex(int fromIndex, int toIndex) {
		...
		for (int i = fromIndex; i < toIndex; i++) {
			...
			} else if (ElementSourceFragment.isSpaceFragment(fragment) && canPrintSpace) {
	...
	protected int findIndexOfNextChildTokenByValue(String token) {
		return ElementSourceFragment.findIndexOfNextFragment(childFragments, childFragmentIdx + 1, fragment -> Objects.equals(token, fragment.getSourceCode()));
	}
	/**
	...
	protected int findIndexOfNextChildTokenByType(TokenType type) {
		return ElementSourceFragment.findIndexOfNextFragment(childFragments, childFragmentIdx + 1, ElementSourceFragment.filter(TokenSourceFragment.class, fragment -> type == fragment.getType()));
	}
	/**
	...
	protected int findIndexOfNextChildTokenOfRole(int start, CtRole role) {
		return ElementSourceFragment.findIndexOfNextFragment(childFragments, start, ElementSourceFragment.checkCollectionItems(ElementSourceFragment.filter(ElementSourceFragment.class, elementFragment -> elementFragment.getRoleInParent() == role)));
	}
	...
	protected int findIndexOfNextChildTokenOfElement(SourcePositionHolder element) {
		return ElementSourceFragment.findIndexOfNextFragment(childFragments, childFragmentIdx + 1, ElementSourceFragment.checkCollectionItems(ElementSourceFragment.filter(ElementSourceFragment.class, elementFragment -> elementFragment.getElement() == element)));
	}
	...
	private int getLastNonSpaceNonCommentBefore(int index, int prevIndex) {
		for (int i = index - 1; i >= 0; i--) {
			SourceFragment fragment = childFragments.get(i);
			if (ElementSourceFragment.isSpaceFragment(fragment)
					|| ElementSourceFragment.isCommentFragment(fragment)
