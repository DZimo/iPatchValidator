...
public class DefaultJavaPrettyPrinter implements CtVisitor, PrettyPrinter {
	...
	public void visitCtFor(CtFor forLoop) {
		...
		if (!st.isEmpty()) {
			try (PrintingContext.Writable _context = context.modify().isFirstForVariable(true)) {
				scan(st.get(0));
			}
		}
		if (st.size() > 1) {
			try (PrintingContext.Writable _context = context.modify().isNextForVariable(true)) {
